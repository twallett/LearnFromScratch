...
def animate_contour(x_k, X, Y, Z, function_latex, f):
    ...
    ax.set_title(f"ConjugateMethod contour plot of {function_latex}")

    line, = ax.plot([], [], 'r', label = 'ConjugateMethod', lw = 1.5)
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    anim.save(os.path.join('plots', "ConjugateMethod_contour.gif"), writer='ffmpeg', fps =1)
...
def animate_surface(x_k, X, Y, Z, function_latex, f):
    ...
    ax.set_title(f"ConjugateMethod surface plot of {function_latex}")
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    anim.save(os.path.join('plots', "ConjugateMethod_surface.gif"), writer='ffmpeg', fps =1)