...
def animate_contour(x_k, X, Y, Z, function_latex, f):
    ...
    ax.set_title(f"SGD contour plot of {function_latex}")

    line, = ax.plot([], [], 'r', label = 'SGD', lw = 1.5)
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    anim.save(os.path.join('plots', "SGD_contour.gif"), writer='ffmpeg')
...
def animate_surface(x_k, X, Y, Z, function_latex, f):
    ...
    ax.set_title(f"SGD surface plot of {function_latex}")
    ...
    anim = animation.FuncAnimation(fig, animate_2, init_func=init_2, frames=len(x_k), 
                                   interval=20,repeat_delay=60, blit=True)
    if not os.path.exists('plots'):
        os.makedirs('plots')
    anim.save(os.path.join('plots', "SGD_surface.gif"), writer='ffmpeg')